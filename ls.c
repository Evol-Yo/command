...
/*
 ...
 Desc : 文件描述信息结构体
	name : 文件名
	stat : 文件的详细信息的结构体，参见linux stat结构体的描述
	linkname : 如果是链接文件的话，这里记录链接文件的名字
	linkmode : 链接到的文件的文件类型
	linkok   : 判断链接文件是否是正确的，这个值影响了链接文件打印的颜色
	filetype : 文件类型
	have_acl : 有无acl权限控制策略
 */
struct fileinfo
  {
    /* The file name. */
...
/*
 * Author : Mingyue
 * Desc   : 将要被处理的目录链表，若realname不为空则为符号链接路径
 */
struct pending
  {
    char *name;
...
/*
 ...
 Modified by : Mingyue
 Desc : 命令输出文件的格式
	long_format   : 列出文件的详细信息
	one_pre_line  : 每行输出一个文件信息
	many_per_line : 以竖直排列的形式，每行输出多个文件
	horizontal    : 以横向排列的形式，每行输出多个文件
	with_commas   : 横向输出文件名，并且中间以逗号作为分隔符
 */
enum format
  {
...
/*
 Author : AOA
 Desc : 时间格式的枚举类型
	full_iso_time_style : 输出全部的ISO8601标准的时间格式
	long_iso_time_style : 只包含年月日时分秒的日期格式 
	iso_time_style      : ISO8601时间格式，前部分年月日，第二部分月日时分秒 
	locale_time_style   : 这个时间格式有些怪，应用中基本用不到
 */
enum time_style
  {
...
/*
 ...
 Modified by : mingyue
 Desc : 按照时间排序方式的枚举类型
	time_mtime : 按照文件数据最后一次被修改的时间排序（默认方式）
	time_ctime : 按照i节点状态最后一次被修改时间排序
	time_atime : 按照文件最后一次被访问的时间进行排序
 */
enum time_type
  {
    time_mtime,			/* default */
...
/*
 ...
 Desc : 排序方式的枚举类型
	sort_none      : 对输出的文件不排序
	sort_name      : 默认的排序方式，按照文件名进行排序
	sort_extension : 按照文件的扩展名进行排序
	sort_time      : 按照时间进行排序
	sort_size      : 按照文件大小进行排序
	sort_version   : 根据版本进行排序
 */
enum sort_type
  {
    sort_none,			/* -U */
...
/*
 *	Author : Mingyue
 *	Desc   : 作为getopt_long函数的参数,记录
 *			了所有可用的命令行参数，关于
 *			option结构体请查看getopt.h头文件
 */
...
Author : AOA
Modified by : Mingyue
Desc   : 从dev_ino_obstack中弹出一个dev_ino结构体
*/
...
Author : AOA
Modified by : Mingyue
Desc   : 将dev 和 ino 组对，存储在ent变量里面，并且插入到active_dir_set表中，
	如果active_dir_set表中已经有了相关的设备和inode的联系，则代表在目录层次
	结构中产生了循环（符号连接），该目录不予处理，然后释放ent、返回false，
	否则返回true
*/
static int
...
/*
Author : AOA
Desc   : 释放掉处于pending状态的文件夹相关结构里面的信息。
*/
static void
...
/*
 * Author : Mingyue
 * Desc   : 用于在接收到中断等信号时在sighandler函数中
 * 			恢复默认的颜色设置
 */
static void
...
/*
Author : AOA
Desc   : 主函数，首先初始化一些系统环境，然后解析用户传进来的参数，
	并且将解析结果赋值到相关全局变量中，返回解析成功的参数的个数i，从i往后既是
	我们要作用命令ls的若干文件的名字。之后主函数会根据全局变量中的值去做相关的动作，
	包括：进一步初始化其他与输出信息相关的变量、进一步判断输出的具体格式等等。
*/
int
...
#define N_ENTRIES(Array) (sizeof Array / sizeof *(Array))
  ...
  /*若命令行参数中指定了路径，则将路径记录在files数组中*/
  for (; i < argc; i++)
    {
      gobble_file (argv[i], unknown, 1, "");
    }

  if (dir_defaulted) /*处理当前工作目录吗？否则命令行参数指定了绝对路径*/
    {
      if (immediate_dirs) /*命令行参数指定有目录吗？*/
	gobble_file (".", directory, 1, ""); /*插入到files数组中*/
      else
	queue_directory (".", 0);/*插入到pending_dirs链表中*/
    }
...
/*
Author : AOA
Desc   : 解析用户输入的参数，并且初始化所有变量。
	第一步，初始化所有与命令相关的全局变量，全部赋值成默认值。
	第二步，解析参数列表，再对相关的全局变量进行赋值。	
*/
static int
decode_switches (int argc, char **argv)
{
  ...
  /*getopt_long:解析命令行参数, 具体查看getopt1.c源文件*/
  while ((c = getopt_long (argc, argv,
	...
	case 'f':
	  ...
	  if (format == long_format)
		/*isatty:测试STDOUT_FILENO描述符是否和终端相关连*/
...
/*
Author : AOA
Desc   : 模拟了一个字符串处理的状态转移自动机，原字符串存储在src里面，目的字符串存储在dest里面
	将src里面的一段人输入的字符串，转移成为计算机编码的字符串，当转移到状态ST_END时停止解析字符串。
*/
static int
...
/*
Author : AOA
Desc   : 用状态机的方式解析颜色方案，并且将解析出来的颜色方案存储到ext当中
	ext应该是一个bin_str结构体变量
*/
static void
parse_ls_color (void)
{
...
Author : AOA
Modified by : Mingyue
Desc   : 将name代表的目录插入到pending_dirs链表中，用于后面对目录的处理
		若realname不为NULL，则代表name为符号连接 
*/
static void
...
Author : AOA
Modified by : Mingyue
Desc   : 处理name目录中的文件，首先检查路径是否出现循环（符号连接），
		其次若进行递归，则将该目录下的子目录添加到pending_dirs链表中，稍
		后处理,然后按指定格式输出该目录下的文件
*/
static void
print_dir (const char *name, const char *realname)
{
  ...
  if (LOOP_DETECT)
	   ...
	   : stat (name, &dir_stat)) < 0)
	{
	 ...
	 we've found a loop, and do not process this directory.  */
	  /* 将dir_stat.st_dev和dir_stat.st_ino放在dev_ino结构体中，然后插入到
	   * active_dir_set表中，如果已经存在，这说明产生了循环，然后返回*/
      if (visit_dir (dir_stat.st_dev, dir_stat.st_ino))
	{
	  ...
	  /* 将dir_stat.st_dev和dir_stat.st.ino放在dev_ino结构体中，然后插入到
	   * dev_ino_obstack表中*/
  ...
  /* files数组中的文件已经添加到pending_dir链表中了， 可以释放了*/
  clear_files ();
  ...
  /*递归的话，就把files数组中的目录添加进pending_dir链表中吧*/
  if (recursive)
...
/*
Author : AOA
Desc   : 将需要忽略匹配的关键字加入到ignore_patterns所指向的链表的头部。
*/
static void
...
/*
Author : AOA
Desc   : 判断next中所代表的文件是否应该被输出，用ignore_pattern中的内容进行筛选。
*/
static int
...
/*
Author : AOA
Desc   : 读取文件相关信息，并且将信息存储到files数组里，并且返回文件所占用的block数量。
*/
static uintmax_t
	     ...
	     const char *dirname)
{
  ...
  if (explicit_arg
      ...
      else
	{
	  ...
	  /* 对于符号链接需要注意的是：stat函数返回由该符号链接的引用文件信息
	   * 而lstat函数返回该符号链接文件本身的信息*/
      switch (dereference)
	{
	case DEREF_ALWAYS:
...
Author : AOA
Midified by : Mingyue
Desc   : 读取filename中的符号链接路径并存储到f->linkname中,如果链接
		无效的话，则出错。
*/
static void
...
/*
Author : AOA
Desc   : 将path和linkname拼接到一起，形成链接文件的路径。
*/
static char *
...
Author : AOA
Modified by : Mingyue
Desc   : 将files数组中的目录放入pending_dirs链表里面，用于后续对目录的处理
	然后将files数组中arg_directory属性的文件记录删除掉。
*/
static void
extract_dirs_from_files (const char *dirname, int ignore_dot_and_dot_dot)
{
  ...
  for (i = files_index - 1; i >= 0; i--)
	...
	if (files[i].name[0] == '/' || dirname[0] == 0)
	  ...
	  {
		/*path_concat : 将路径dirname和files[i]拼接起来*/
...
/*
Author : AOA
Desc   : 首先按照inode修改时间进行比较，时间相同比较文件名。
*/
static inline int
...
Author : AOA
Modified by : Mingyue
Desc   : 首先按照最后一次修改文件数据的时间进行排序，再按照文件名进行比较。
*/
static inline int
...
Author : AOA
Modified by : Mingyue
Desc   : 首先按照文件最后一次被访问的时间进行排序，再按照文件名进行排序。
*/
static inline int
...
/*
Author : AOA
Desc   : 根据用户设置的排序方式，对文件列表(files数组)进行排序。
*/
static void
sort_files (void)
{
...
/*
Author : AOA
Desc   : 计算在long-format输出格式中，时间戳一列需要占几个字符。
*/
static int
...
/*
Author : AOA
Desc   : 格式化user列的数据，如果有用户名则输出用户名，否则输出uid。
*/
static size_t
...
/*
Author : AOA
Desc   : 按照详细信息的格式打印信息，设置-l选项时调用此函数。
*/
static void
...
Author : AOA
Modified by : Mingyue
Desc   : 以用户设置的文件名的输出格式输出文件，例如用""或[]将文件名括起来
*/
static void
...
/*
Author : AOA
Desc   : 在没有设置C_END这个颜色的时候，
	用C_LEFT + C_NORM + C_RIGHT三者的颜色代替 C_END。
*/
static void
...
Author : AOA
Modified by : Mingyue
Desc   : 打印文件名和一些周边信息，周边信息包括：
	inode的信息，block_size和用户定制的文件名的输出格式
*/
static void
...
/*
 Author : AOA
 Desc : 根据文件的类型mode和name所对应的系统定义的颜色方案，输出相关的颜色。
 */
static void
...
/*
 Author : AOA
 Desc : 将ind里面的string内容输出到STDOUT_FILENO所指向的文件中去。
 */
static int
...
/*
 Author : AOA
 Modified by : Mingyue
 Desc 	: 根据用户的定制计算输出文件信息所占字符，包括inode信息、block_size信息
 		和设置quoting_option后，文件名所占用的字符等
 */
static int
...
/*
 ...
 Modified by : Mingyue
 Desc : 以竖直排列的形式，输出多行。
		column_info[i],表示每行放i+1列文件，结构体中
			line_len   : 表示每一列最右端到行首的距离，用字符数表示
			valid_len  : 表示每行是否能输出i+1个文件
			???col_arr[j] : 表示每行输出的第j个文件最多占多少个字符
		
		此函数分为两步进行
			第一步确定每行最多可以输出多少个文件名。
			第二步根据第一步的结果，按行输出每一行中所应该输出的文件名。
 */
static void
  ...
  for (filesno = 0; filesno < files_index; ++filesno)
      ...
      for (i = 0; i < max_cols; ++i)
	{
	  if (column_info[i].valid_len)
	    {
	      int idx = filesno / ((files_index + i) / (i + 1)); //确定filesno号文件在第几列，((files_index + i) / (i + 1))确定可以输出几行，用filesno比上输出几行即得到filesno所对应的列号。
	      int real_length = name_length + (idx == i ? 0 : 2);//判断是不是每行最后一个文件，如果是，后面就不需要多输出两个空格，所以real_length = name_length，否则 real_length = name_length + 2
...
/*
 Author : AOA
 Desc : 按列输出，横向排序的输出方式。
		原理和 print_many_per_line 差不多，只不过在计算idx的时候有些差别，
		以及在最终输出的时候，print_many_per_line 是以列为主，而此函数是以行为主。
		两个函数在输出的时候文件名均采用左对齐的方式。
 */
static void
...
/*
 Author : AOA
 Desc : 按行输出文件名，文件名中间用逗号进行分隔。原理简单，不再赘述。
 */
static void
...
/*
 Author : AOA
 Desc : 在from 到 to之间的位置上补全空白符
		首先用tab字符进行补全，最后不够一个tab字符的空间用空格补全。
 */
static void
...
/*
 Author : AOA
 Desc : 连接dirname和name的内容，并把相关内容复制到dest所指向的存储空间中
		首先处理dirname，需要注意的一点是，处理完后，dest必须以'/'结尾，如果不是以这个文件名字的分隔符结尾，则补齐
		第二步将name的内容再拷贝到dest当中。
...
/*
 Author : AOA
 Desc : 初始化column_info数组，
		column_info[i],表示每行放i+1列文件，结构体中
			line_len   : 表示每列最右端距离行首的距离
			valid_len  : 表示每行是否能输出i+1个文件
			???col_arr[j] : 表示每行输出的第j个文件最多占多少个字符
 */
static void
...
/*
 Author : AOA
 Desc : 输出ls命令的使用说明
	status : 输出信息的种类，status != 0 的时候，输出帮助命令的调用方式，否则输出ls的详细使用信息。
 */
void
usage (int status)
{
